...
always_inline static func f32 abs(const f32 p_x) {
    return p_x < 0 ? -p_x : p_x;
}
always_inline static func f64 abs(const f64 p_x) {
    return p_x < 0 ? -p_x : p_x;
}

always_inline static func f32 min(const f32 p_x, const f32 p_y) {
    return p_x < p_y ? p_x : p_y;
}
always_inline static func f64 min(const f64 p_x, const f64 p_y) {
    return p_x < p_y ? p_x : p_y;
}

always_inline static func f32 max(const f32 p_x, const f32 p_y) {
    return p_x > p_y ? p_x : p_y;
}
always_inline static func f64 max(const f64 p_x, const f64 p_y) {
    return p_x > p_y ? p_x : p_y;
}

always_inline static func f32 clamp(const f32 p_value, const f32 p_min, const f32 p_max) {
    return p_value < p_min ? p_min : (p_value > p_max ? p_max : p_value);
}
always_inline static func f64 clamp(const f64 p_value, const f64 p_min, const f64 p_max) {
    return p_value < p_min ? p_min : (p_value > p_max ? p_max : p_value);
}

always_inline static func f32 trunc(const f32 p_x) {
    return to_f32(to_i32(p_x));
}
always_inline static func f64 trunc(const f64 p_x) {
    return to_f64(to_i64(p_x));
}

always_inline static func f32 decimal(const f32 p_x) {
    return abs(p_x - trunc(p_x));
}
always_inline static func f64 decimal(const f64 p_x) {
    return abs(p_x - trunc(p_x));
}

always_inline static func f32 floor(const f32 p_x) {
    const f32 t = trunc(p_x);
    return t > p_x ? t - 1.0f : t;
}
always_inline static func f64 floor(const f64 p_x) {
    const f64 t = trunc(p_x);
    return t > p_x ? t - 1.0 : t;
}

always_inline static func f32 ceil(const f32 p_x) {
    const f32 t = trunc(p_x);
    return t < p_x ? t + 1.0f : t;
}
always_inline static func f64 ceil(const f64 p_x) {
    const f64 t = trunc(p_x);
    return t < p_x ? t + 1.0 : t;
}

always_inline static func f32 round(const f32 p_x) {
    return p_x >= 0.0f ? floor(p_x + 0.5f) : ceil(p_x - 0.5f);
}
always_inline static func f64 round(const f64 p_x) {
    return p_x >= 0.0 ? floor(p_x + 0.5) : ceil(p_x - 0.5);
}

always_inline static func f32 cross(const f32 p_x1, const f32 p_x2, const f32 p_y1, const f32 p_y2) {
     return p_x1 * p_x2 - p_y1 * p_y2;
}
always_inline static func f64 cross(const f64 p_x1, const f64 p_x2, const f64 p_y1, const f64 p_y2) {
     return p_x1 * p_x2 - p_y1 * p_y2;
}

always_inline static func f32 dot(const f32 p_x1, const f32 p_x2, const f32 p_y1, const f32 p_y2) {
     return p_x1 * p_x2 + p_y1 * p_y2;
}
always_inline static func f64 dot(const f64 p_x1, const f64 p_x2, const f64 p_y1, const f64 p_y2) {
     return p_x1 * p_x2 + p_y1 * p_y2;
}

always_inline static func f32 sin_series(const f32 p_x) {
...
always_inline static func f64 sin_series(const f64 p_x) {
...
always_inline static func f32 cos_series(const f32 p_x) {
    const f32 x2 = p_x * p_x;
    constexpr const f32 y1 = 1.0f;
...
always_inline static func f64 cos_series(const f64 p_x) {
    const f64 x2 = p_x * p_x;
    constexpr const f64 y1 = 1.0;
...
always_inline static func f32 reduce_2pi(const f32 p_x) {
    return p_x - trunc(p_x * (f32(1.0f) / F32_TAU) + (p_x < 0 ? -0.5f : 0.5f)) * F32_TAU;
}
always_inline static func f64 reduce_2pi(const f64 p_x) {
    return p_x - trunc(p_x * (f64(1.0) / F64_TAU) + (p_x < 0 ? -0.5 : 0.5)) * F64_TAU;
}

always_inline static func f32 sin_half(const f32 p_x) {
...
always_inline static func f64 sin_half(const f64 p_x) {
...
always_inline static func f64 cos(const f64 p_x) {
    const f64 x = reduce_2pi(p_x);
...
always_inline static func f32 tan(const f32 p_x) {
    const f32 x = reduce_2pi(p_x);
...
always_inline static func f64 tan(const f64 p_x) {
    const f64 x = reduce_2pi(p_x);
